...
import type { Consensus, ModelVote, ProposedAction } from "./governance";
...
export async function runConsensusReasoner(
  ...
  const votes: ModelVote[] = voters.map((v, i) => {
    ...
    if (r !== undefined && r.status === "fulfilled") {
      return { model: v.proposer.label, kind: r.value.kind, justification: r.value.justification };
    }
    return {
  ...
  const consensus: Consensus = { votes, agreementRatio, chosenKind };
  return {
    kind: chosen.kind,
